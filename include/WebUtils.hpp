...
namespace WebUtils
{
	...
	/// @param finished the callback for when we're done downloading
	/// @param progressUpdate callback to give a progressupdate bar or something
	void GetAsyncBytes(std::string url, long timeout, std::function<void(long, std::shared_ptr<std::vector<uint8_t>>)> finished, std::function<void(float)> progressUpdate = nullptr);
}
