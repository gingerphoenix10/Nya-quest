...
custom_types::Helpers::Coroutine NyaUtils::ImageView::DownloadImage(
    StringW url,
    ...
    std::function<void(bool success, long HTTPCode)> finished)
{
  ...
  {
    UnityEngine::Networking::UnityWebRequest* webRequest = UnityEngine::Networking::UnityWebRequestTexture::GetTexture(imageURL);
    ...
    QuestUI::MainThreadScheduler::Schedule([this, finished, imageURL, url]
      {
  ...
  {
    // Direct download of the texture for images
    UnityEngine::Networking::UnityWebRequest* webRequest = UnityEngine::Networking::UnityWebRequestTexture::GetTexture(imageURL);

    co_yield reinterpret_cast<System::Collections::IEnumerator*>(CRASH_UNLESS(webRequest->SendWebRequest()));

    if(webRequest->get_isNetworkError()){
      co_return finished(false, 200);
    }

    auto downloadHandlerTexture = reinterpret_cast<UnityEngine::Networking::DownloadHandlerTexture*>(webRequest->get_downloadHandler());
