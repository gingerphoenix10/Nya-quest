...
UnityEngine::UI::Image* createImage(UnityEngine::UI::Image* img, UnityEngine::Sprite* sprite, UnityEngine::Transform* parent){
    ...
    img->set_material(Nya::Main::NyaFloatingUI->UINoGlow);
...
Nya::NoteUI::NoteUI(UnityEngine::Transform* parent, UnityEngine::Sprite* noteArrowSprite, int rotation){
    rot = rotation;
    auto* transform = QuestUI::BeatSaberUI::CreateCanvas()->get_transform();
    transform->set_localScale({0.46f, 0.46f, 0.0f});
    noteBackground = createImage(noteBackground, QuestUI::BeatSaberUI::Base64ToSprite(Nya::Sprites::bloq), transform);
    noteArrow = createImage(noteArrow, noteArrowSprite, transform);
    cutDistanceImage = createTexture(cutDistanceImage, transform);
    noteCutArrow = createImage(noteCutArrow, QuestUI::BeatSaberUI::Base64ToSprite(Nya::Sprites::cut_arrow), transform);
    ...
    hint->dyn__hoverHintController() = nullptr;
